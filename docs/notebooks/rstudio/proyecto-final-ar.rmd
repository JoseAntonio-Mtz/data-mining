...
Escribe la fecha actual (**NOTE-01**): 03 de junio de 2021
...
```{r}
NOMBRE_COMPLETO = "José Antonio Martínez Caldera"
DIGITOS_MATRICULA = 01374995
```
...
```{r}
try(
  plot(smalltx, main = paste0("Elaborado por: Jose Antonio Martinez Caldera", " (", 01374995, ")" ))  
)
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): No hay relación del producto de pañales con ningún otro artículo mostrado.
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 0.20
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 0.33
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque las métricas de "support" y "confidence" excluyen a la regla `{beer} => {nuts}`
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Representa cada una de las sesiones del data set; además de que su objetivo es distinguir el # de sesiones que los usuarios tienen en spotify para poder hacer recomendaciones
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Representa cada una de las canciones para distinguir cuál es qué canciones que escuchan ciertos usuarios para hacer recomendaciones con base en sus preferencias. 
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, ya que representan el comportamiento que el usuario tiene en Spotify al momento de escuchar música. Es posible hacer asociaciones si se comprende la relación de las canciones con los gustos a través del número de pausas por canción, así como canciones saltadas; entre otras. 
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): ID: t_bacf06d3-9185-4183-84ea-ff0db51475ce; Frec. abs:901
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): Itemset(20) = 1609
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Los usuarios tienen en promedio 14.64 sesiones en promedio.
...
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? : Es que se pierda precisión en el análisis por lo que se tendrá un mayor número de reglas de asociación que pueden no ser lo más certeras posibles. 
...
- Provee el enlace al video aquí: **NOTE-30** Enlace para video: https://drive.google.com/drive/folders/1qIpKrAbmCMaZ9D4sxFGoTrNfT1TXzwUY?usp=sharing
